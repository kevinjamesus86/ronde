...
async function formatToolResult(toolkit, toolName, result, ctx) {
	...
	const rendered = formatter ? renderWithFormatter(formatter, result) : defaultFormatter(toolName, result);
...
function renderWithFormatter(formatter, result) {
	if (result.ok) return formatter(result.data);
	if (result.data === void 0) return result.error;
...
async function* executeToolCalls(input) {
	...
	async function finalize(i, result) {
		...
		try {
			...
			content = `Formatter failed: ${e.message}`;
			result = err(content);
		}
		settled[i] = {
			result,
