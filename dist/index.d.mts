...
//#endregion
//#region packages/core/src/block.d.ts
/**
 * @module
...
type Block = TextBlock | BinaryBlock | RefBlock;
declare function text(s: string): TextBlock;
declare function image(data: string | URL, mediaType: string): BinaryBlock;
declare function file(data: string | URL, opts: {
    mediaType: string;
    filename?: string;
}): BinaryBlock;
declare function audio(data: string | URL, mediaType: string): BinaryBlock;
declare function ref(uri: string, opts?: {
    mediaType?: string;
    bytes?: number;
    summary?: string;
}): RefBlock;
 ...
 */
declare function blocksToText(blocks: readonly Block[]): string;
//#endregion
...
type MessagePart = ContentPart | ThinkingPart | ToolCallPart | ToolResultPart;
/**
...
declare function userMessage(content: string | Block[], meta?: unknown): Message;
declare function assistantMessage(parts: MessagePart[], id?: string): Message;
declare function toolResultMessage(toolCallId: string, ok: boolean, content: string | Block[], meta?: unknown): Message;
/**
 * Build a ContentPart. Accepts a string (wrapped to `[text(s)]`) or
 * a pre-built `Block[]`. The string form is the 90% case.
 */
declare function contentPart(role: Role, content: string | Block[], meta?: unknown): ContentPart;
/**
 * Convenience: build a single-text-block ContentPart. Equivalent to
 * `contentPart(role, content, meta)` when content is a string.
 */
declare function textPart(role: Role, content: string, meta?: unknown): ContentPart;
...
declare abstract class Workspace {
    ...
    abstract spill(content: string | Uint8Array, opts?: SpillOpts): Promise<SpillResult>;
}
/**
...
type ToolResult<D = unknown> = Result<D>;
...
type ToolExecuteReturn<D = unknown> = Promise<ToolResult<D>> | AsyncGenerator<string, ToolResult<D>, void>;
 ...
 */
type ToolFormatterFn = (data: unknown) => string | Block[];
/**
...
declare function defaultFormatter(_toolName: string, result: ToolResult): string;
/**
 ...
 */
declare function formatToolResult(toolkit: Toolkit<any>, toolName: string, result: ToolResult, ctx?: FormatContext): Promise<Block[]>;
 ...
 * ```
 */
type ExecuteFn<Args, Ctx, D> = (args: Args, ctx: Ctx) => Awaitable<ToolResult<D>> | AsyncGenerator<string, ToolResult<D>, void>;
 ...
 */
declare function coalesceByRole<M, T>(messages: NormalizedMessage<M>[], mapRole: (role: Role) => string, serializePart: (part: NormalizedPart<M>) => T | T[] | undefined): Array<{
    role: string;
    parts: T[];
}>;
//#endregion
...
type ProgressEvents = {
    ...
    tool_result: {
        turn: number;
        call: ToolCall; /** Formatted blocks the model sees — journaled as the ToolResultPart content. */
        content: Block[]; /** Raw structured exec return. Never journaled. */
        result: ToolResult;
    };
};
 ...
 */
declare const readFile: (pathCtx: PathContext) => Toolkit<Workspace>;
//#endregion
 ...
 */
declare const writeFile: (pathCtx: PathContext) => Toolkit<Workspace>;
//#endregion
 ...
 */
declare const editFile: (pathCtx: PathContext) => Toolkit<Workspace>;
//#endregion
 ...
 */
declare const globFiles: (pathCtx: PathContext, opts?: GlobOptions) => Toolkit<Workspace>;
//#endregion
 ...
 */
declare const grepFiles: (pathCtx: PathContext, opts?: GrepOptions) => Toolkit<Workspace>;
//#endregion
 ...
 */
declare const listDirectory: (pathCtx: PathContext, opts?: ListOptions) => Toolkit<Workspace>;
//#endregion
 ...
 *   a pre-built one.
 */
declare const shell: (pathCtx: PathContext, opts?: ShellOptions) => Toolkit<Workspace>;
//#endregion
 ...
 * with `merge()`.
 */
declare function coreTools(opts: CoreToolsOptions): Toolkit<Workspace>;
//#endregion
...
declare class FsWorkspace extends Workspace {
    #private;
    readonly id: string;
    readonly kind: "fs";
    get dir(): string;
    ...
    [rebase](dir: string): void;
    spill(content: string | Uint8Array, o?: SpillOpts): Promise<SpillResult>;
}
//#endregion
...
interface RunObserver {
    ...
    onToolResult?(turn: number, toolCall: ToolCall, content: Block[], result: ToolResult): void;
...
//#endregion
export { AgentStep, AgentStepToolCall, AgenticConfig, AgenticResult, AgenticStreamConfig, Awaitable, type BackendConfig, BinaryBlock, Block, BlockKind, type CompactionContext, type CompactionResult, type CompactionStrategy, CompletionBackend, CompletionDelta, CompletionError, CompletionErrorKind, CompletionRequest, CompletionResponse, CompletionWarning, ConfiguredBackend, ContentPart, CoreToolsOptions, DEFAULT_MAX_CONTEXT, DEFAULT_MAX_INLINE, DEFAULT_MAX_OUTPUT, DefaultCompactionOptions, DefaultCompactionStrategy, DiagnosticEvents, EMPTY_USAGE, type EditFileData, Effort, EngineConfig, EngineDiagnosticEvent, EngineEvent, EngineEventKind, EngineHooks, EngineLifecycleEvent, EngineProgressEvent, EngineResult, FormatContext, FsJournal, type FsRuntime, FsRuntimeStat, FsWorkspace, type GlobData, type GrepData, type GrepMatch, type InternalBackendConfig, Journal, JournalEvent, JournalSink, Lax, LifecycleEvents, type ListDirectoryData, type ListDirectoryEntry, ManagedRuntimeOptions, Message, MessagePart, MessageType, NormalizedMessage, NormalizedPart, PathContext, type PathSpec, PreStepInput, PreStepResult, ProgressEvents, type ProviderDescriptor, type ReadFileData, RefBlock, ResolvedBackendConfig, Result, RetryOptions, RetryingBackend, Role, type RootSpec, type RunObserver, RunTotals, type Runtime, type SandboxConfig, type SettleReason, type ShellData, type ShellKind, type Snapshot, SpillOpts, SpillResult, StateConfig, StatefulToolContext, StopReason, TextBlock, ThinkingPart, ToolCall, ToolCallPart, ToolContext, ToolDefBase, ToolExecuteReturn, ToolExecutor, ToolFormatterFn, ToolResult, ToolResultPart, ToolSchema, Toolkit, TruncateStrategy, UsageStats, Workspace, type WriteFileData, agentic, agenticStream, allProviders, asGenerator, assistantMessage, audio, bindToolkitRuntime, blocksToText, canonicalize, classifyError, coalesceByRole, contentPart, coreTools, createBackend, createFsRuntime, createManagedRuntime, createRuntime, defaultFormatter, diagnosticEvent, drain, editFile, emptyUsage, engine, err, estimateTokens, file, formatToolResult, genHex, genId, generate, getProvider, globFiles, grepFiles, hydrate, image, isAsyncGenerator, isOk, lifecycleEvent, listDirectory, merge, ok, openFsRuntime, openRuntime, partRole, progressEvent, readFile, ref, registerProvider, replay, resolveRuntimeResources, resume, ro, rw, sanitizeFilename, shell, statFsRuntime, text, textPart, thinkingPart, tool, toolCallPart, toolResultMessage, toolResultPart, userMessage, utf8ByteLength, withRetry, wrapSdkError, writeFile };
